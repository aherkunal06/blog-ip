...
export default function AdminLoginPage() {
  ...
  const handleOTPLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // OTP verified successfully, now create session using NextAuth
      console.log('Creating session for admin:', verifyData.admin.username);
      
      // Use NextAuth signIn with OTP token
      const otpToken = JSON.stringify({ verified: true });
      
      console.log('Calling signIn with:', {
        username: verifyData.admin.username,
        hasOtpToken: !!otpToken,
        otpTokenPreview: otpToken.substring(0, 50),
      });
      ...
      if (res?.ok) {
        ...
        console.error('NextAuth signIn failed:', res);
        const errorMessage = res?.error || 'Unknown error';
        console.error('Error details:', errorMessage);
        toast.error(`OTP verified but session creation failed: ${errorMessage}. Please try again.`);
      }
    } catch (error) {
